...
namespace Mirle.DB.Proc
{
    public class clsErrorReport
    {
            ...
            try
            {
                using (var db = clsGetDB.GetDB(_config))
                {
                    ...
                    if (iRet == DBResult.Success)
                    {
                        ...
                        {
                            buffer.OnAlarmTrigger += Buffer_OnAlarmTrigger2;
                            buffer.OnAlarmClear += Buffer_OnAlarmClear2;
                        }
                ...
                using (var db = clsGetDB.GetDB(_config))
                {
                    ...
                    if (iRet == DBResult.Success)
                    {
                        ...
                        if (CVC_Alarm.GetAlarmCVCInfo(PLCNO, sBufferIndex, hAlarmBit, out var alarminfo, db) == GetDataResult.Success)
                        {
                            ...
                            {
                                clear(alarminfo, db, hAlarmBit, e);
                            }
        ...
        private void clear(DataObject<AlarmCVCInfo> alarminfo,SqlServer db,string hAlarmBit,AlarmEventArgs e)
        {
